...
2. **Curl / HTTP script** against the running Vite dev server.
3. **CLI invocation** with a fixture input, diffing stdout against a known-good
   snapshot.
4. **Headless browser script** (Playwright) — drives the game UI,
   asserts on DOM/console/network.
5. **Browser DevTools trace.** Capture console logs, network requests, and
   DOM mutations; replay a captured session through the code path in isolation.
6. **Replay a captured trace.** Save a real user interaction log to disk;
   replay it through the code path in isolation.
7. **Throwaway harness.** Spin up a minimal subset of the system (one game
   module, mocked deps) that exercises the bug code path with a single
   function call.
...
dump, screen recording with timestamps), or (c) permission to add
