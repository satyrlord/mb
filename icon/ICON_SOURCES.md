# Approved Icon Sources (CC / Free)

Use this file to register allowed sources before importing assets.

| Source | URL | License | Allowed For Project | Notes |
| --- | --- | --- | --- | --- |
| OpenMoji | <https://openmoji.org/> | CC BY-SA 4.0 | Yes | Attribution required |
| Twemoji (graphics) | <https://twemoji.twitter.com/> | CC BY 4.0 | Yes | Attribution required |
| Wikimedia Commons | <https://commons.wikimedia.org/> | Varies per asset | Conditional | Verify each asset license |

## Intake Checklist

1. Verify license is compatible (CC0, CC BY, CC BY-SA preferred).
2. Record every imported file in `ATTRIBUTION.csv`.
3. Keep source URL and author intact.
4. Do not import assets with unclear or conflicting terms.
5. Prefer SVG resources for visual consistency.

## OpenMoji Folder Policy

- Store all OpenMoji SVG files in `icon/openmoji/svg/`.
- Do not create per-import batch folders.
- For future imports, add files directly into `icon/openmoji/svg/`
  and append rows in `ATTRIBUTION.csv` using paths under
  `icon/openmoji/svg/`.
